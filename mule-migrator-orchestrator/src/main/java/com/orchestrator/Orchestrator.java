...
public class Orchestrator {
	...
	public static void main(String[] args) {
		...
		if (args.length > 0 && args.length == 4) {
			...
			logger.info(
					"Provide command line arguments in format : -projectBasePath <mule 3base path> -destinationProjectBasePath <mule 4 projects>");
			return;
		}
	...
	private static void generateLiteEstimate(String sourcePath, String destinationPath) {
		...
		try {
			List<Path> files = Files.walk(sourceDir).filter(Files::isRegularFile)
					.filter(p -> p.toFile().getName().contains(".json")).collect(Collectors.toList());
			...
			for (Path file : files) {
				...
				CSVUtil.writeToCSV(DESTINATION_PROJECTS_BASE_PATH + File.separator + DESTINATION_CSV_FILE, metaDataBean,
						false);
	...
	private static void generateEstimate(String PROJECT_BASE_PATH, String DESTINATION_PROJECT_BASE_PATH,
		...
		try {
			...
			mr.parseMMAReport(DESTINATION_PROJECT_BASE_PATH + File.separator + filePathBuilder(reportFiles),
					metaDataBean);
		} catch (Exception e) {
		...
		logger.debug(
				"am.getTotalMELLineExpressionsPendingMigration(): " + am.getTotalMELLineExpressionsPendingMigration());
	...
	private static String getDomainProjectName(String pROJECT_BASE_PATH) {
		...
		try (InputStream input = new FileInputStream(
				pROJECT_BASE_PATH + File.separator + filePathBuilder(DEPLOY_FILE_REL_PATH))) {
